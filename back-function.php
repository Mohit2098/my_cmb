...
function load_posts_by_category() {
    ...
    if ($post_query->have_posts()) {
        ...
        while ($post_query->have_posts()) : $post_query->the_post();
            ...
            if (($post_count + 1) % 3 == 0) {
                echo '</div>';
                // Only open a new row if there are more posts to display
                if ($post_count + 1 < $post_query->post_count) {
                    echo '<div class="row hm-mn-rw">';
                }
            }
...
//     $posts_per_page = isset($_POST['posts_per_page']) ? intval($_POST['posts_per_page']) : 6;
    ...
    if ($query->have_posts()) {
        ...
        while ($query->have_posts()) : $query->the_post();
            ...
            if ($posts_in_row % 4 == 0) {
                echo '</div>'; // Close the current row
                echo '<div class="row w-100 trd-rw-cl">'; // Start a new row for the next set of posts
                $posts_in_row = 0; // Reset the counter for the new row
            }
